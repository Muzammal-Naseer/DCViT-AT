...
def BIT(pretrained=True):
    model = ResNetV2(ResNetV2.BLOCK_UNITS['r50'], width_factor=1, head_size=1000)
    if pretrained:
        weights_IN = np.load("pretrained_models/R50x1_224.npz")
        model.load_from(weights_IN)
    return model

def BIT_160(pretrained=True):
    model = ResNetV2(ResNetV2.BLOCK_UNITS['r50'], width_factor=1, head_size=1000)
    if pretrained:
        weights_IN = np.load("pretrained_models/R50x1_160.npz")
        model.load_from(weights_IN)
    return model