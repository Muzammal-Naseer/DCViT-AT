...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,  which_prompt=224, text_project=False):
        ...
        layer_wise_tokens = [x.permute(1, 0, 2) for x in layer_wise_tokens]  # LND -> NLD

        # x_resPrompt, x = x[:, 0:self.num_prompts].mean(dim=1), x[:, self.num_prompts]

        # x = self.ln_post(x[:, 0, :])
        layer_wise_tokens = [self.ln_post(x) for x in layer_wise_tokens]
        ...
        return layer_wise_tokens, layer_wise_tokens
