...
class resPromptVisionTransformer(Images_VIT):
    ...
    def forward(self, x, all_tokens=True):
        ...
        return (x_resPrompt, x_cls_prompt), ([(xi[:,(T + self.num_prompts)]) for xi in layer_wise_tokens_norm], [(xi[:, 0:(T + self.num_prompts)].mean(dim=1)) for xi in layer_wise_tokens_norm]), attention_maps  # , x
        # return x_resPrompt, [xi[:, (T + self.num_prompts)] for xi in layer_wise_tokens_norm], attention_maps  # , x

class resPromptDino2(Images_VIT):
    ...
    def forward(self, x, all_tokens=True):
        ...
        x_cls2 = ([(x_c) for x_c in x_cls], [(x_rp) for x_rp in x_resPrompt])
        return x_resPrompt2, (x_cls_ens, x_resPrompt_ens)   # , x


class resPromptClip2(nn.Module):
