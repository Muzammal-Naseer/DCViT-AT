...
class Block(nn.Module):
    ...
    def forward(self, pixel_embed, patch_embed,return_attention=False):
        # inner
        y, _ = self.attn_in(self.norm_in(pixel_embed))
        pixel_embed = pixel_embed + self.drop_path(y)
        pixel_embed = pixel_embed + self.drop_path(self.mlp_in(self.norm_mlp_in(pixel_embed)))
        # outer
        B, N, C = patch_embed.size()
        patch_embed[:, 1:] = patch_embed[:, 1:] + self.proj(self.norm1_proj(pixel_embed).reshape(B, N - 1, -1))
        y, attn =  self.attn_out(self.norm_out(patch_embed))
        patch_embed = patch_embed + self.drop_path(y)
        patch_embed = patch_embed + self.drop_path(self.mlp(self.norm_mlp(patch_embed)))
        if return_attention:
            return pixel_embed, patch_embed, attn
...
class TNT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            pixel_embed, patch_embed, attn = blk(pixel_embed, patch_embed,return_attention=True)
            layer_wise_tokens.append(patch_embed)
