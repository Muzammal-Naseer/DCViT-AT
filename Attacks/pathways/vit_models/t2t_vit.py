...
class T2T_ViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        return [x[:, 0] for x in layer_wise_tokens], layer_wise_tokens, attention_maps

    def forward(self, x, only_last=False,  all_tokens=False):
        token_out, layer_wise_tokens, attention_maps = self.forward_features(x)
        x = [self.head(x) for x in token_out]
        if only_last:
