        ...
        for idx, image_label in prog_bar:
            ...
            if idx == IDX:
                ...
                if not video_data:
                    ...
                    org1 = img[:,:,4,:,:]
                    adv1 = adv[:,:,4,:,:]
                    vutils.save_image(vutils.make_grid(adv1, normalize=True, scale_each=True), f'{args.dir}/adv.png')
                    vutils.save_image(vutils.make_grid(org1, normalize=True, scale_each=True), f'{args.dir}/org.png')
                    diff = np.clip((org1-adv1).detach().cpu(), 0, 1) * scale
                    ...
                    # plt.imshow(scale * diff.permute(1,0,2,3))
                    # plt.savefig(f'{args.dir}/diff.png')
                    vutils.save_image(vutils.make_grid((diff), normalize=True, scale_each=True), f'{args.dir}/diff.png')
                    return
