...
for ATTACK in 'mifgsm' 'pgd' 'fgsm'; do
  evaluation 'ucf101' "deit_base_image" "timesformer_vit_base_patch16_224"  "" "$ATTACK" -1 20 16 "all" "$None" "pretrained_video_models/timesformer/ucf/8_div/results/checkpoint_epoch_00015.pyth"
done

for ATTACK in 'mifgsm' 'pgd' 'fgsm'; do
  evaluation 'ucf101' "deit_small_image" "timesformer_vit_base_patch16_224"  "" "$ATTACK" -1 20 16 "all" "$None" "pretrained_video_models/timesformer/ucf/8_div/results/checkpoint_epoch_00015.pyth"
done

for ATTACK in 'mifgsm' 'pgd' 'fgsm'; do
  evaluation 'ucf101' "deit_tiny_image" "timesformer_vit_base_patch16_224"  "" "$ATTACK" -1 20 16 "all" "$None" "pretrained_video_models/timesformer/ucf/8_div/results/checkpoint_epoch_00015.pyth"
done
