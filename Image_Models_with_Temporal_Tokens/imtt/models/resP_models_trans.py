...
__all__ = [
    ...
    'deit_tiny_patch16_224_resPT_100', 'deit_small_patch16_224_resPT_100', 'deit_base_patch16_224_resPT_100',
]
...
def deit_tiny_patch16_224_resPT_10(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.load('pretrained_models/deit_tiny_patch16_224_resPT_10Prompts_96/checkpoint.pth',
                                map_location="cpu"
                                )
        model.load_state_dict(checkpoint["model"])
    return model
...
def deit_small_patch16_224_resPT_10(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.load('pretrained_models/deit_small_patch16_224_resPT_10Prompts_96/checkpoint.pth',
                                map_location="cpu"
                                )
        model.load_state_dict(checkpoint["model"])
    return model
...
@register_model
def deit_tiny_patch16_224_resPT_100(pretrained=False, **kwargs):
    model = resPromptVisionTransformer(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
...
@register_model
def deit_small_patch16_224_resPT_100(pretrained=False, **kwargs):
    model = resPromptVisionTransformer(
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),num_prompts=100, **kwargs)
    model.default_cfg = _cfg()
...
@register_model
def deit_base_patch16_224_resPT_100(pretrained=False, **kwargs):
    model = resPromptVisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), num_prompts=100,  **kwargs)
    model.default_cfg = _cfg()
...
if __name__=="__main__":
    model = create_model(
        'deit_base_patch16_224_resPT_100',
        pretrained=False,
        num_classes=1000,
        drop_rate=0.0,
        drop_path_rate=0.1,
