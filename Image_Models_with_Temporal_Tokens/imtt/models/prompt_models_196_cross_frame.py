...
from .timesformer_block import Block, PatchEmbed, CrossBlock
...
class resPromptVisionTransformer(Images_VIT):
    def __init__(self, *args,actual_num_classes=400,qk_scale=None, num_prompts=1, num_frames=8, img_size=224, attention_type='divided_space_time', num_cross=1, **kwargs):
        ...
        self.num_prompts = num_frames
        self.attention_type = attention_type
        num_frames = num_frames
        img_size = img_size
        self.cross_blocks = num_cross
        self.num_frames = num_frames
        # print(self.cross_blocks)
        ...
        cblocks = []
        for i in range(self.cross_blocks):
            cblocks.append(CrossBlock(dim=embed_dim, num_heads=num_heads, mlp_ratio=4, qkv_bias=True, qk_scale=qk_scale, drop=0., attn_drop=0., drop_path=dpr[i], norm_layer=partial(nn.LayerNorm, eps=1e-6)))
        self.transformation = nn.ModuleList(cblocks)
        # print(len(cblocks))
    ...
    def forward_features(self, x):
        ...
        assert resPrompt_token.shape[1] == self.num_frames
        # print(resPrompt_token[2,3,45])

        ## transformation if the input is a video
        if not is_image:
            x = rearrange(x[:,:,:], '(b t) n m -> b t n m',t=T)
            final_prompt = torch.Tensor().cuda()
            for j in range(self.num_frames):
                latent = resPrompt_token[:,j,:].unsqueeze(1)
                context = x[:,j,:,:]
                for i, blk in enumerate(self.transformation):
                    latent = blk(latent, context)
                final_prompt = torch.cat((final_prompt, latent), dim=1)

            resPrompt_token = final_prompt
...
class resPromptClip(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, num_prompts: int, num_classes:int, num_frames: int , actual_num_classes: int, attention_type, num_cross: int, qk_scale=None):
        super().__init__()
        self.num_prompts = num_frames
        ...
        cblocks = []
        for i in range(self.cross_blocks):
            cblocks.append(CrossBlock(dim=self.width, num_heads=heads, mlp_ratio=4, qkv_bias=True, qk_scale=qk_scale, drop=0., attn_drop=0., drop_path=dpr[0], norm_layer=partial(nn.LayerNorm, eps=1e-6)))
        self.transformation = nn.ModuleList(cblocks)
