...
class resPromptVisionTransformer(Images_VIT):
    def __init__(self, *args,actual_num_classes=400,qk_scale=None, num_prompts=1, num_frames=8, img_size=224, attention_type='divided_space_time', num_cross=1, sample=4, **kwargs):
    ...
    def forward_features(self, x):
        ...
        if not is_image:
            ...
            if isinstance(self.transformation, nn.Sequential):
                for block in self.transformation:
                    transformation = block(x[:,:,:],B,T,W,is_cls=True)
            else:
    ...
    def forward(self, x, all_tokens=True, individual_token=None):
        # print(x.shape)
        size = x.size(0)
        ...
        if is_image:
            x = [self.head(x[:, (1 + self.num_prompts)]) for x in layer_wise_tokens_norm]
        # print("IND", individual_token)
        if individual_token is not None:
            assert individual_token < (T + self.num_prompts)
            x_resPrompt = [self.head_resPrompt(x[:, individual_token]) for x in layer_wise_tokens_norm]
        else:
            # x_resPrompt = [self.head_resPrompt(x[:, 0:(T + self.num_prompts)].mean(dim=1)) for x in layer_wise_tokens_norm]
            x_resPrompt = [self.head_resPrompt(x[:, (T + self.num_prompts)]) for x in layer_wise_tokens_norm]
        if is_image:
...
class resPromptDino(Images_VIT):
    def __init__(self, *args,actual_num_classes=400,qk_scale=None, num_prompts=1, num_frames=8, img_size=224, attention_type='divided_space_time', sample=4, **kwargs):
        super().__init__(*args,**kwargs)
...
class resPromptClip(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, num_prompts: int, num_classes:int, num_frames: int , actual_num_classes: int, attention_type, qk_scale=None, sample=4):
        super().__init__()
        self.num_prompts = num_prompts
