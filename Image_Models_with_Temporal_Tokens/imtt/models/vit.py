...
class vit_base_patch16_224_timeP_1(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.pretrained=True
        patch_size = 16
        self.model = resPromptVisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE,num_classes=1000, actual_num_classes=cfg.MODEL.NUM_CLASSES, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES, attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, num_cross=cfg.MODEL.NUM_CROSS, **kwargs)
        ...
        # self.model.pre_vit_norm.weight.data = self.model.norm.weight.data.clone()
        # self.model.pre_vit_norm.bias.data = self.model.norm.bias.data.clone()
        # self.model.head_resPrompt.weight.data = self.model.head.weight.data.clone()
        # self.model.head_resPrompt.bias.data = self.model.head.bias.data.clone()

    def forward(self, x):
        x_resPrompt, layer_wise_tokens, attention_maps = self.model(x)
    ...
    def __init__(self, cfg, **kwargs):
        ...
        if self.pretrained:
            checkpoint = torch.load(
                "timesformer/models/pretrained/deit_base_patch16_224-b5f2ef4d.pth"
            )
...
@MODEL_REGISTRY.register()
class vit_base_patch16_224_timeP_10(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(vit_base_patch16_224_timeP_10, self).__init__()
        print("HELLOOO")
        self.pretrained=True
        patch_size = 16
        self.model = resPromptVisionTransformer( img_size=cfg.DATA.TRAIN_CROP_SIZE,num_prompts=10,num_classes=1000, actual_num_classes=cfg.MODEL.NUM_CLASSES, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES, attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, **kwargs)
...
@MODEL_REGISTRY.register()
class vit_base_patch16_224_timeP_100(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(vit_base_patch16_224_timeP_100, self).__init__()
        self.pretrained=True
        patch_size = 16
        self.model = resPromptVisionTransformer( img_size=cfg.DATA.TRAIN_CROP_SIZE,num_prompts=100,num_classes=1000, actual_num_classes=cfg.MODEL.NUM_CLASSES, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES, attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, **kwargs)
...
@MODEL_REGISTRY.register()
class dino_base_patch16_224_10P(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(dino_base_patch16_224_10P, self).__init__()
        self.pretrained=True
        patch_size = 16
        self.model = resPromptDino(img_size=cfg.DATA.TRAIN_CROP_SIZE,num_classes=1000, num_prompts=10,actual_num_classes=cfg.MODEL.NUM_CLASSES, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES, attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, **kwargs)
...
@MODEL_REGISTRY.register()
class dino_base_patch16_224_100P(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(dino_base_patch16_224_100P, self).__init__()
        self.pretrained=True
        patch_size = 16
        self.model = resPromptDino(img_size=cfg.DATA.TRAIN_CROP_SIZE,num_classes=1000, num_prompts=100,actual_num_classes=cfg.MODEL.NUM_CLASSES, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=cfg.DATA.NUM_FRAMES, attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, **kwargs)
...
@MODEL_REGISTRY.register()
class clip_base_patch16_224_10P(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(clip_base_patch16_224_10P, self).__init__()
        self.pretrained=True
        patch_size = 16
        self.model = resPromptClip(
                input_resolution = 224,
...
@MODEL_REGISTRY.register()
class clip_base_patch16_224_100P(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(clip_base_patch16_224_100P, self).__init__()
        self.pretrained=True
        patch_size = 16
        self.model = resPromptClip(
                input_resolution = 224,
