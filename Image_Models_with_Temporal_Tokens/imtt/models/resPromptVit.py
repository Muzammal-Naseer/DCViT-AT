...
def compute_rollout_attention(all_layer_matrices, start_layer=0):
    ...
    print(all_layer_matrices[-1].shape, all_layer_matrices[1].shape, all_layer_matrices[2].shape)
    num_tokens = all_layer_matrices[0].shape[1]
    batch_size = all_layer_matrices[0].shape[0]
    eye = torch.eye(num_tokens).expand(batch_size, num_tokens, num_tokens).to(all_layer_matrices[0].device)

    # transformation_matrix = all_layer_matrices[-1] + torch.eye(num_tokens - 2).expand(batch_size, num_tokens - 2, num_tokens - 2).to(all_layer_matrices[-1].device)
    ...
    # joint_attention = transformation_matrix.bmm(joint_attention[:,2:])
...
class resPromptVisionTransformer(VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        if not is_image:
            ...
            ## prompt token
            resPrompt_token = self.add([self.resPrompt_token.expand(B, -1, -1), transformation])
    ...
    def forward(self, x, all_tokens=True):
        ...
        if not is_image:
            return x_resPrompt, layer_wise_tokens, attention_maps, is_image  # , x
        else:
            x = [self.head(x[:, 0]) for x in layer_wise_tokens_norm]
            return x, layer_wise_tokens, attention_maps, is_image
    
    def relprop(self, cam=None,method="transformer_attribution", is_ablation=False, start_layer=0, num_prompts=1, which_prompt=0, is_image=False, **kwargs):
        if not is_image:
            cam = self.head_resPrompt.relprop(cam, **kwargs)
        else:
        ...
        if method == "full":
            ...
            if not is_image:
                cam_prompt = cam[:, 0:num_prompts, (num_prompts + 1):].mean(dim=1)
            else:
                cam_prompt = cam[:, 0, (num_prompts + 1):].mean(dim=1)
            cam_cls = cam[:,num_prompts, (num_prompts + 1):]
            return cam_prompt, cam_cls 
        ...
        # our method, method name grad is legacy
            ...
            if not is_image:
                cam_prompt = rollout[:, 0:num_prompts, (num_prompts + 1):].mean(dim=1)
            else:
                cam_prompt = rollout[:, 0, (num_prompts + 1):]
            cam_cls = rollout[:,num_prompts, (num_prompts + 1):]
            return cam_prompt, cam_cls
            ...
            if is_ablation:
                ...
                cam = grad * cam
            cam = cam.clamp(min=0).mean(dim=0)
            cam_prompt = cam[0:num_prompts, (num_prompts + 1):].mean(dim=0)
            cam_cls = cam[num_prompts, (num_prompts + 1):]
            return cam_prompt, cam_cls 
