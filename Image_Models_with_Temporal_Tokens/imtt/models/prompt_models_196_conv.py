...
class resPromptVisionTransformer(Images_VIT):
    def __init__(self, *args,actual_num_classes=400,qk_scale=None, num_prompts=1, num_frames=8, img_size=224, attention_type='divided_space_time', **kwargs):
        ...
        self.aggr_conv = nn.Conv3d(num_frames,1,3,1,1)
    ...
    def forward_features(self, x):
        ...
        # x = rearrange(x[:,1:,:], '(b t) n m -> b (n t) m',b=B,t=T)

        x = rearrange(x[:,1:,:], 'b (t n_h n_w) m -> b t n_h n_w m',b=B,t=T,n_w=W)

        x = self.aggr_conv(x).squeeze(1)
        x = rearrange(x, 'b n_h n_w m -> b (n_h n_w) m',n_w=W)
        x = torch.cat((cls_tokens, x), dim=1)
