...
import imtt.models.losses as losses
import imtt.models.optimizer as optim
import imtt.utils.checkpoint as cu
import imtt.utils.distributed as du
import imtt.utils.logging as logging
import imtt.utils.metrics as metrics
...
from imtt.utils.meters import TrainMeter, ValMeter
from imtt.utils.multigrid import MultigridSchedule
...
def train(cfg):
    ...
    # Freeze entire model except view token and view head
    for name,param in model.named_parameters():
        # param.requires_grad = False
        if "transformation" not in name and "Prompt" not in name and "time_embed" not in name and "temp_conv" not in name:  
            param.requires_grad = False
            # param.grad = None 

    for name,param in model.named_parameters():
        print(name, type(param), param.shape, param.requires_grad, param.grad)
    
    num = sum(p.numel() for p in model.parameters() if p.requires_grad)
    num2 = sum(p.numel() for p in model.parameters())
    print(num, num2)
